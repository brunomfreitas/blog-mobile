...
import escolaridade from "../../../constants/escolaridade";
...
export default function QuestaoForm() {
  ...
  const [serie, setSerie] = useState(escolaridade);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (isEdit) {
          const resp = await getQuestaoById(String(id));
          const p = resp?.data ?? resp;

          setTitle(p?.title ?? "");
          setSubtitle(p?.subtitle ?? "");
          setMessage(p?.message ?? "");
          setSerie(p.escolaridade ?? []);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        ...
        router.replace(`/(shell)/questao/${String(id)}`);
      } else {
        ...
        if (newId) router.replace(`/(shell)/questao/${String(newId)}`);
        else router.back();
      }
    } catch (e) {
  ...
  return (
    <>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <Text style={styles.h1}>{isEdit ? "Editar Questão" : "Nova Questão"}</Text>
        <Text style={styles.h2}>Preencha os campos abaixo para {isEdit ? "atualizar" : "criar"} uma questão.</Text>
		...
		*/}
        ...
        <View style={styles.pills}>
          {escolaridade.map((s) => {
            const active = Number(serie) === Number(s.id);
            return (
              <TouchableOpacity key={s.id} onPress={() => setSerie(s.id)} style={[styles.pill, active && styles.pillActive]}>
