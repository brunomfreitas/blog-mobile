...
import { deleteQuestao, getAllQuestao, searchQuestao } from "../../../services/questaoService";
...
function normalizePost(p) {
	...
	return {
		...
		resposta: p?.resposta ?? "",
		categoryName: p?.questaoMaterial?.name ?? p?.categoryName ?? "Geral",
		status: p?.status === true ? 'Ativo' : 'Inativo',
		escolaridade: p?.escolaridade ?? null,
		turno: p?.turno ?? null,
		periodo: p?.periodo ?? null,
		tipoQuestao: p?.tipoQuestao ?? null,
		createdAt: p?.createdAt ?? null,	
		author: p?.createdByPerson?.name ?? p?.author?.name ?? "—",
		postedAt: p?.postedAt ?? null
	};
}

export default function QuestaoManage() {
	...
	async function fetchPosts({ reset = false, nextPage = 1 } = {}) {
		try {
			...
			if (q) {
				resp = await searchQuestao(q); // se seu backend retornar lista simples
			} else {
				resp = await getAllQuestao(token, nextPage, 12);
			}
	...
	const renderItem = ({ item }) => {
		...
		<View style={styles.card}>
			...
			<Text style={styles.title} numberOfLines={20}>Enunciado: {item.enunciado}</Text>
			...
			<TouchableOpacity
				style={[styles.btn, styles.btnGhost]}
				onPress={() => router.push(`/(private)/questao/edit/${item.id}`)}
			>
	...
	return (
		...
		<View style={styles.topRow}>
			<Text style={styles.pageTitle}>Banco de Questões</Text>

			<TouchableOpacity
			style={styles.newBtn}
			onPress={() => router.push("/(private)/questao/new")}
			>
